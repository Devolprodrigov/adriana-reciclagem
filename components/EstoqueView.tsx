...
import { Search, Scale, ArrowUp, Plus } from 'lucide-react';
import { Product } from '../types';
import { db } from '../firebase'; // GARANTA QUE ESSE CAMINHO ESTÁ CORRETO
...
const EstoqueView: React.FC<Props> = ({ products, notify }) => {
  ...
  const handleSaveNewItem = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Tenta gravar na coleção 'products' do Firestore
      ...
      // Se travar aqui, o alert vai te dizer exatamente o porquê
      alert("FALHA AO GRAVAR: " + (error.message || "Verifique as Rules do Firebase"));
    } finally {
      // ISSO AQUI DESTRAVA O BOTÃO OBRIGATORIAMENTE
      setIsSaving(false);
    }
  };
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-80">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-2xl font-bold text-sm outline-none focus:ring-2 ring-indigo-500 transition-all" 
            />
          </div>
          <button 
            onClick={() => setShowModal(true)} 
            className="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-2 hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all"
          >
        ...
        {products
          ...
          .map(p => (
            <div key={p.id} className={`bg-white p-8 rounded-[2.5rem] border ${p.stock <= p.minStock ? 'border-rose-100 bg-rose-50/20' : 'border-slate-100'} shadow-sm flex flex-col items-center transition-all hover:shadow-md`}>
              ...
              <h4 className="font-black text-slate-800 uppercase text-center text-sm line-clamp-1">{p.name}</h4>
              ...
              <div className="w-full flex gap-2">
                ...
                <button 
                  ...
                  className="p-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-50"
                >
                  <ArrowUp size={16}/>
                </button>
              </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={handleSaveNewItem} className="grid grid-cols-2 gap-4">
              ...
              <button 
                type="submit" 
                disabled={isSaving} 
                className="col-span-2 py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs mt-4 shadow-xl shadow-indigo-100 hover:bg-indigo-700 disabled:opacity-50 transition-all"
              >
              ...
              <button 
                ...
                className="col-span-2 text-slate-300 font-bold text-[10px] uppercase hover:text-slate-500 transition-colors"
              >
                Cancelar
              </button>
            </form>
