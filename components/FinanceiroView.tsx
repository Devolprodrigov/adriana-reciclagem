...
const FinanceiroView: React.FC<Props> = ({ financials, notify }) => {
  ...
  const handlePDFImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // BUSCA ROBUSTA PELO MOTOR: tenta encontrar em qualquer uma dessas variáveis globais
    const pdfjsLib = (window as any)['pdfjs-dist/build/pdf'] || (window as any).pdfjsLib;
    
    if (!pdfjsLib) {
      notify("O motor de PDF está terminando de carregar. Clique novamente em 2 segundos.");
      return;
    }
    ...
    reader.onload = async (event) => {
      try {
        ...
        // Configura o Worker usando a mesma versão da CDN
        ...
        for (let i = 1; i <= pdf.numPages; i++) {
          ...
          // Regex para Extrato Itaú e similares (Data DD/MM + Descrição + Valor R$)
        ...
        notify(importedCount > 0 ? `Sucesso! ${importedCount} lançamentos importados.` : "Nenhuma transação clara encontrada no PDF.");
      } catch (err) {
        console.error("Erro no PDF:", err);
        notify("Erro ao processar o arquivo. Tente um PDF de extrato original.");
      } finally {
  ...
  const handleAddRecord = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      notify("Erro ao salvar lançamento.");
    }
  };
