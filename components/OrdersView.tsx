  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      batch.set(financialRef, {
        ...
        description: `${orderType.toUpperCase()} - ${selectedPartner.name}`,
        value: total,
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in">
      <div className="lg:col-span-8 space-y-6">
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
          <div className="flex justify-between items-center border-b border-slate-50 pb-6">
             <div className="flex bg-slate-100 p-1.5 rounded-2xl gap-1">
               <button onClick={() => setOrderType('compra')} className={`px-8 py-2.5 rounded-xl text-xs font-black uppercase ${orderType === 'compra' ? 'bg-white text-emerald-600 shadow-md' : 'text-slate-400'}`}>Compra</button>
               <button onClick={() => setOrderType('venda')} className={`px-8 py-2.5 rounded-xl text-xs font-black uppercase ${orderType === 'venda' ? 'bg-white text-indigo-600 shadow-md' : 'text-slate-400'}`}>Venda</button>
             </div>
              ...
              {customerSearch && !selectedPartner && (
                <div className="absolute bg-white shadow-xl rounded-2xl z-50 w-full p-2 border border-slate-100 mt-1">
                  {allPartners.slice(0, 5).map(p => (
                    <button key={p.id} onClick={() => { setSelectedPartner(p); setCustomerSearch(p.name); }} className="w-full text-left p-3 hover:bg-slate-50 rounded-xl font-bold text-sm">{p.name}</button>
                  ))}
                </div>
              )}
          </div>
        ...
        <div className="space-y-4">
          <div className="relative">
            ...
            <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Pesquisar material..." className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl font-bold text-xs outline-none" />
          </div>
      ...
      <div className="lg:col-span-4">
        <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-xl sticky top-8">
          <h4 className="font-black text-lg mb-6 uppercase tracking-tight">Checkout</h4>
          <div className="space-y-3 mb-8 max-h-80 overflow-y-auto pr-2">
            {cart.map(item => (
              <div key={item.product.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl">
                <div><p className="font-black text-[10px] uppercase">{item.product.name}</p><p className="text-xs font-bold text-indigo-600">{item.quantity} kg</p></div>
          ...
          <div className="border-t pt-6">
            ...
            <button onClick={handleFinish} disabled={cart.length === 0 || !selectedPartner} className="w-full py-5 bg-indigo-600 text-white rounded-[2rem] font-black uppercase text-xs disabled:opacity-20 transition-all">Finalizar Pedido</button>
          </div>
        </div>
      </div>
    </div>
